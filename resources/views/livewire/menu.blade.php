<nav x-data="{ open: false }" class="bg-gray-800 border-b border-gray-700">
                ...
                @foreach($menuItems as $item)
                    ...
                    @else
                        ...
                        <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                            <x-nav-link :href="route($item['route'])" :active="request()->routeIs($item['route'] . '.*') || request()->routeIs($item['route'])" wire:navigate>
                                {{ __($item['label']) }}
                            </x-nav-link>
                        </div>
                    @endif
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @foreach($menuItems as $item)
                @if(isset($item['children']))
                    ...
                    <div x-data="{ open{{ $loop->index }}: false }">
                        <button @click="open{{ $loop->index }} = !open{{ $loop->index }}"
                ...
                @else
                    <x-responsive-nav-link :href="route($item['route'])" :active="request()->routeIs($item['route'] . '.*') || request()->routeIs($item['route'])" wire:navigate>
                        {{ __($item['label']) }}
                    </x-responsive-nav-link>
                @endif
            @endforeach
        ...
        <div class="pt-4 pb-1 border-t border-gray-700">
            <div class="px-4">
                <div class="font-medium text-base text-white">{{ $userName }}</div>
                <div class="font-medium text-sm text-gray-400">{{ $userEmail }}</div>
            </div>
